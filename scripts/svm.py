...
x_list = ['sigmoid', 'rbf', 'linear', 'poly']
...
for kernel in x_list:
    ...
    clf = SVC(kernel=kernel, cache_size=200, gamma='scale')
    ...
    filename = "../heart/svm/" + fileout + "/output.txt"
    ...
    title = "Learning Curve: Heart Disease Data, SVM - " + fileout
    plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))

    plt2.savefig("../heart/svm/" + fileout + "/learning_curve.png", bbox_inches='tight')
    plt2.close()
    ...
    plt.savefig("../heart/svm/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
    plt.close()
    out += [newOut]

with open("../heart/svm/MASTER.csv", "w") as f:
...
# plt.savefig("../heart/svm/MASTER.png")
# plt.show()