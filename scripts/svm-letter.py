...
x_list = ['sigmoid', 'rbf', 'linear', 'poly', 'precomputed']

for kernel in x_list:
    ...
    fileout = "Kernel Type -- " + kernel
    clf = SVC(kernel=kernel, cache_size=2, gamma='scale')
    ...
    # graph.overlap ='scale'
    # graph.render("../letter//" + fileout + "/tree")

    newOut += [accuracy_score(testY, clf.predict(testX))]
    csvout += [[kernel, str(accuracy_score(testY, clf.predict(testX)))]]
    ...
    filename = "../letter-recognition/svm/" + fileout + "/output.txt"
    ...
    title = "Learning Curve: Letter Data, SVM Net - " + fileout
    plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))

    plt2.savefig("../letter-recognition/svm/" + fileout + "/learning_curve.png", bbox_inches='tight')
    plt2.close()
    ...
    plt.savefig("../letter-recognition/svm/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
    plt.close()
    out += [newOut]

with open("../letter-recognition/svm/MASTER.csv", "w") as f:
