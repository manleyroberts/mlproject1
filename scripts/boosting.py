...
x_list = list(range(1, 25, 5))
y_list = list(range(10, 261, 60))
...
for max_depth in x_list:
    ...
    for max_leaf_nodes in y_list:
        ...
        fileout = "Max Depth - " + str(max_depth) + ", Max Leaves - " + str(max_leaf_nodes)
        clf = tree.DecisionTreeClassifier(criterion="gini", max_depth=max_depth, max_leaf_nodes=max_leaf_nodes)
        clf = AdaBoostClassifier(base_estimator=clf, n_estimators=5, learning_rate=1.0, algorithm='SAMME.R')
        ...
        # graph.overlap ='scale'
        # graph.render("../heart/boosting/" + fileout + "/tree")
        ...
        filename = "../heart/boosting/" + fileout + "/output.txt"
        ...
        title = "Learning Curve: Grad Admissions Data, Boosted DTree - " + fileout
        plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))

        plt2.savefig("../heart/boosting/" + fileout + "/learning_curve.png", bbox_inches='tight')
        plt2.close()
        ...
        s.xaxis.set_ticklabels(['N', 'P'])
        s.yaxis.set_ticklabels(['N', 'P']);

        plt.savefig("../heart/boosting/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
        plt.close()
    out += [newOut]

with open("../heart/boosting/MASTER.csv", "w") as f:
...
plt.savefig("../heart/boosting/MASTER.png")
plt.show()