...
for k in x_list:
    ...
    csvout += [[k, str(accuracy_score(testY, clf.predict(testX)))]]
    ...
    filename = "../letter-classification/knn/" + fileout + "/output.txt"
    ...
    title = "Learning Curve: Letter Classification Data, K Nearest Neighbors - " + fileout
    plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))

    plt2.savefig("../letter-classification/knn/" + fileout + "/learning_curve.png", bbox_inches='tight')
    plt2.close()
    ...
    plt.savefig("../letter-classification/knn/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
    plt.close()
    out += [newOut]
    print(newOut)

with open("../letter-classification/knn/MASTER.csv", "w") as f:
    f.write("num_layers, layer_size, accuracy_score\n")
    for row in csvout:
        f.write(str(row[0]) + ", " + str(row[1])+ "\n")

plt.plot(x_list, [x[0] for x in out])
plt.xlabel('K')
plt.ylabel('accuracy_score')
plt.savefig("../letter-classification/knn/MASTER.png", bbox_inches='tight')
...
# plt.savefig("../letter-classification/knn/MASTER.png")
# plt.show()