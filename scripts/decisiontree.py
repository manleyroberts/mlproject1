...
for max_depth in x_list:
    ...
    for max_leaf_nodes in y_list:
        ...
        #     newRow = []
        #     for x in r:
        #         newRow = newRow + [int(float(x)*100)]
        #     newRows = newRows + [newRow]
        #     newRow[-1] = int(newRow[-1] > 72)
        # rows = newRows
        ...
        graph.overlap ='scale'
        graph.render("../heart/decision/" + fileout + "/tree")
        ...
        with open("../heart/decision/" + fileout + "/output.txt", "w") as f:
        ...
        title = "Learning Curve: Heart Disease Data, Decision Tree - " + fileout
        plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))

        plt2.savefig("../heart/decision/" + fileout + "/learning_curve.png", bbox_inches='tight')
        plt2.close()
        ...
        plt.savefig("../heart/decision/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
        plt.close()
    out += [newOut]

with open("../heart/decision/MASTER.csv", "w") as f:
...
plt.savefig("../heart/decision/MASTER.png")
plt.show()