...
x_list = list(range(8, 24, 8))
y_list = list(range(100, 300, 100))
...
filename = "../letter-classification/decision/" + fileout + "/output.txt"
...
title = "Learning Curve: Letter Recognition, Decision Tree - " + fileout
plt2 = plot_learning_curve(clf, title, trainX, trainY, (0.0, 1.1), cv=5, n_jobs=4, train_sizes=np.linspace(.1, 1.0, 5))
plt2.show()
# plt2.savefig("../letter-recognition/decision/" + fileout + "/learning_curve.png", bbox_inches='tight')
plt2.close()
...
plt.show()
# plt.savefig("../letter-recognition/decision/" + fileout + "/confusion_matrix.png", bbox_inches='tight')
plt.close()

# with open("../letter-recognition/decision/MASTER.csv", "w") as f:
...
# plt.savefig("../letter-recognition/decision/MASTER.png")
# plt.show()